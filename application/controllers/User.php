...
class User extends CI_Controller {
    ...
    public function index(){ 
        ...
        if(isset($_POST['form_login'])) {
            ...
            $screenwidth = $this->security->xss_clean($this->input->post('width', true));
            ...
            if(!$chk) {
            	...
            	$array = array(
                    ...
                    'uprofilePicture'=> $chk['profile_picture'],
         ...
         if ($gClient->getAccessToken()) {
             ...
             if($this->user_model->google_user_exist($userProfile['id']))
             {
                 $userdata = [
                     ...
                     'is_login_session_id'=>$session_id
                 ];
             ...
             $array = array(
                ...
                'uprofilePicture'=> $profileData['profile_picture'],
                'ukennelLogo'=> $profileData['kennel_logo'],
                'is_user_logged_in' =>'yes',
                'is_google_user' =>'yes',
                'session_id' => $session_id,
                'currencySymbol'=>	$getDetails->geoplugin_currencySymbol
    ...
    public function register_user(){ 
        ...
        $data['dogBreeds']   = $this->mastersetting_model->kennel_type_list();   
    ...
    public function save_user(){ 
        ...
        if(isset($_POST['form_registration'])) {
            ...
            $confirm_password = $this->security->xss_clean($this->input->post('conf_password', true));
            $dog_breed = $this->security->xss_clean($this->input->post('dog_breed', true));
            ...
            if($valid==1){
                ...
                $form_data = array(
                    ...
                    'kennel_type_id'    =>($dog_breed)?$dog_breed:'',
                    
	            );
                
	            $responsId = $this->user_model->registration($form_data);
                $path = $_FILES['kennel_logo']['name'];
                $path_tmp = $_FILES['kennel_logo']['tmp_name'];
                if($path!='') {
                    $ext = pathinfo( $path, PATHINFO_EXTENSION );
                    $file_name = basename( $path, '.' . $ext );
                
                    $final_name = 'klogo-'.$responsId.'.'.$ext;
                    move_uploaded_file($path_tmp, 'uploads/kennellogo/'.$final_name);
                    $saveLogo=array(
                        'kennel_logo'=> $final_name,
                        'user_id'=>$responsId
                    );
                    $this->user_model->update_registration($saveLogo);
                }
